...
public class EmpleadoBeanDAO extends MantenimientoGeneralDAO<Empleado> implements EmpleadoBeanDAOLocal {
	...
	public int grabar(Empleado parametros) {
		...
		case Constantes.INSERTAR:
			sql.append( " insert into empleado (curso_id, dni, nombres, ape_pate, ape_mate, direccion, telefono, celular, email, estado, tipo_emp, remunera, usu_reg, fec_reg, maq_reg, colegio_id ) values "+Utiles.inParametros(16));	
			break;

		case Constantes.ACTUALIZAR:
			sql.append( " update empleado set curso_id=?, dni=?, nombres=?, ape_pate=?, ape_mate=?, direccion=?, telefono=?, celular=?, email=?, estado=?, tipo_emp=?, remunera=?, usu_mod=?, fec_mod=?, maq_mod=?, colegio_id=? where empleado_id=? ");	
			break;
		default:
			break;
		}
...
//			prepaStatement.setDate(i++, parametros.getFecReg());
			...
			prepaStatement.setInt(i++, parametros.getColegioId());
			break;
